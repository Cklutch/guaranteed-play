...
SPORTSBOOK = "FanDuel"
DEFAULT_SOURCE_PATH = Path("data/raw/fanduel_props.csv")
    ...
    try:
        ...
        if props_df.empty:
            return {
                ...
                "messages": ["No FanDuel props loaded."],
            }
        ...
        return {
            ...
            "missing_market_count": 0,
            "missing_line_count": 0,
            "messages": [f"FanDuel validation failed: {exc}"],
        }


def load_fanduel_props(source_path=DEFAULT_SOURCE_PATH):
...
def get_source_debug_info(source_path=DEFAULT_SOURCE_PATH):
    props_df = load_fanduel_props(source_path=source_path)
